...
Fix failing tests from a previous `/coverit:run`, `/coverit:full`, or `/coverit:verify`. Reads failure details from `.coverit/progress/` files, uses AI to refine test code, and re-executes only the failed plans.
