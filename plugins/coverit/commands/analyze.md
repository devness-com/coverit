---
description: "AI-driven codebase analysis — creates coverit.json quality manifest"
---

# Coverit Analyze

AI explores the project with tool access (Glob, Grep, Read, Bash) to create the `coverit.json` quality manifest. Discovers modules, maps existing tests, classifies complexity, identifies user journeys and API contracts.
...
The analysis produces a large manifest. **You MUST delegate this to a sub-agent** using the Task tool.
...
```
Analyzing codebase. This may take a few minutes...
```
...
**CRITICAL**: The sub-agent MUST use the `mcp__plugin_coverit_coverit__coverit_analyze` MCP tool.
