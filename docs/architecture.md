...
coverit is built on a 4-command architecture centered around `coverit.json` as the single source of truth.
...
CLI / MCP
    │
    ├── scan ────→ AI explores codebase with tools ──→ creates coverit.json
    │
...
readManifest()               Read coverit.json
        │
        ▼
runExistingTests()           Execute test suite
        │
        ▼
AI provider.generate()       For failures: AI diagnoses and fixes test code
        │                    Tools: Read, Glob, Grep, Bash, Write, Edit
        ▼
scanTests()                  Rescan filesystem for test files
        │
        ▼
rescoreManifest()            Recalculate quality score
        │
